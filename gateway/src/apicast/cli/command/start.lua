...
local function openresty_binary(candidates)
  return resty_env.value("APICAST_OPENRESTY_BINARY") or resty_env.value("TEST_NGINX_BINARY") or
...
local function build_context(options, config)
  ...
  context.ca_bundle =
    pl.path.abspath(tostring(context.ca_bundle) or pl.path.join(context.prefix, "conf", "ca-bundle.crt"))
...
local function configure(cmd)
  ...
  cmd:option("-3 --channel", "3scale configuration channel to use.", channel):action(
    function(args, name, chan)
  ...
  cmd:option(
    "-e --environment",
    "Deployment to start. Can also be a path to a Lua file.",
    resty_env.value("APICAST_ENVIRONMENT")
  ):count("*"):init(loaded_env):action("concat"):convert(load_env)
  cmd:flag("--development --dev", "Start in development environment"):action(
    function(arg, name)
  ...
  cmd:option("-c --configuration", "Path to custom config file (JSON)", resty_env.value("APICAST_CONFIGURATION"))
  ...
  do
    ...
    cmd:mutex(
      cmd:flag("-b --boot", "Load configuration on boot.", configuration_loader == "boot"):action(
        set_configuration_loader("boot")
      ):target("configuration_loader"):init(configuration_loader),
      cmd:flag("-l --lazy", "Load configuration on demand.", configuration_loader == "lazy"):action(
        set_configuration_loader("lazy")
      ):target("configuration_loader"):init(configuration_loader)
    )
  end
  cmd:option(
  ...
  cmd:mutex(
    cmd:flag("-v --verbose", "Increase logging verbosity (can be repeated)."):count(
      ("0-%s"):format(#(_M.log_levels) - _M.log_level)
    ),
    cmd:flag("-q --quiet", "Decrease logging verbosity."):count(("0-%s"):format(_M.log_level - 1))
  )
