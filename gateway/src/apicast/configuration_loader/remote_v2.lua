...
local services_subset = function()
  ...
  if resty_env.value("APICAST_SERVICES") then
    ngx.log(
      ngx.WARN,
...
function _M:oidc_issuer_configuration(service)
  ...
  if not config then
    ngx.log(
      oidc_log_level,
      "invalid OIDC Provider, expected application/json got:  ",
  ...
  if not issuer then
    ngx.log(
      oidc_log_level,
      "invalid OIDC Issuer, expected application/json got:  ",
      res.headers.content_type,
      " body: ",
      res.body
    )
...
function _M:config(service, environment, version)
  ...
  if not id then
    return nil, "invalid service, missing id"
  end
