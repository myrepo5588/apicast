...
function _M.configured(configuration, host)
  if not configuration or not configuration.find_by_host then
    return nil, "not initialized"
  end
...
local boot = {
  rewrite = noop,
  ttl = function()
    return tonumber(env.value("APICAST_CONFIGURATION_CACHE"), 10)
  end
}
...
function boot.init(configuration)
  ...
  if config and init then
    ...
    ngx.log(
      ngx.EMERG,
      "failed to load configuration, exiting (code ",
      code,
      ")\n",
