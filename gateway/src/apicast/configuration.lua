...
local function build_policy_chain(policies)
  if not policies or policies == null then
    return nil, "no policy chain"
  end
  ...
  return Service.new(
    {
      ...
      oauth_login_url = type(proxy.oauth_login_url) == "string" and len(proxy.oauth_login_url) > 0 and
        proxy.oauth_login_url or
        nil,
      secret_token = proxy.secret_token,
      hostname_rewrite = type(proxy.hostname_rewrite) == "string" and len(proxy.hostname_rewrite) > 0 and
...
function _M.services_limit()
  ...
  if env.value("APICAST_SERVICES") then
    ngx.log(
      ngx.WARN,
      "DEPRECATION NOTICE: Use APICAST_SERVICES_LIST not APICAST_SERVICES as this will soon be unsupported"
    )
  end
  if not subset or subset == "" then
    return services
  end
...
function _M.filter_services(services, subset)
  ...
  if not subset or not next(subset) then
    return services
  end
