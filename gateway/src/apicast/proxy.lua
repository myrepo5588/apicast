...
function _M:authorize(service, usage, credentials, ttl)
  if not usage or not credentials then
    return nil, "missing usage or credentials"
  end
...
function _M.get_upstream(service)
  ...
  local host, port, path = url[4], url[5] or resty_url.default_port(url[1]), url[6] or ""
...
local function sync_post_action(self, cached_key, service, credentials, formatted_usage)
  local backend = assert(backend_client:new(service, http_ng_ngx), "missing backend")
  local res = backend:authrep(formatted_usage, credentials, response_codes_data(), self.extra_params_backend_authrep)
  ...
  if ok then
    if type(c) == "table" and type(c.setup) == "function" then
      ...
      ngx.log(
        ...
        custom_config,
        " because it does not return table with function setup"
      )
    end
  else
